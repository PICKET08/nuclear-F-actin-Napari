...
#         return torchvision.transforms.functional.pad(img, padding, fill=self.fill)
...
class PadToSize(object):
    def __init__(self, target_size, fill=0):
        ...
        self.fill = fill
        self.padding = None  # 记录 (left, top, right, bottom)
    ...
    def __call__(self, img):
        ...
        return torchvision.transforms.functional.pad(img, self.padding, fill=self.fill)

def remove_padding(tensor, padding):
    """
    tensor: torch.Tensor, shape (C, H, W) or (H, W)
    padding: (pad_left, pad_top, pad_right, pad_bottom)
    """
