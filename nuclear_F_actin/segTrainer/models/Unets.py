...
import torch
def align_and_concat(*tensors):
    target_size = tensors[0].shape[2:]
    aligned = [F.interpolate(t, size=target_size, mode='bilinear', align_corners=True) for t in tensors]
...
class U_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        d5 = self.Up5(e5)
        d5 = align_and_concat(e4, d5)

        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        d4 = align_and_concat(e3, d4)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        d3 = align_and_concat(e2, d3)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        d2 = align_and_concat(e1, d2)
        d2 = self.Up_conv2(d2)
...
class AttU_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        d5 = align_and_concat(x4, d5)
        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        x3 = self.Att4(g=d4, x=e3)
        d4 = align_and_concat(x3, d4)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        x2 = self.Att3(g=d3, x=e2)
        d3 = align_and_concat(x2, d3)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        x1 = self.Att2(g=d2, x=e1)
        d2 = align_and_concat(x1, d2)
        d2 = self.Up_conv2(d2)
...
class NestedUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x0_1 = self.conv0_1(align_and_concat(x0_0, self.Up(x1_0)))

        x2_0 = self.conv2_0(self.pool(x1_0))
        x1_1 = self.conv1_1(align_and_concat(x1_0, self.Up(x2_0)))
        x0_2 = self.conv0_2(align_and_concat(x0_0, x0_1, self.Up(x1_1)))

        x3_0 = self.conv3_0(self.pool(x2_0))
        x2_1 = self.conv2_1(align_and_concat(x2_0, self.Up(x3_0)))
        x1_2 = self.conv1_2(align_and_concat(x1_0, x1_1, self.Up(x2_1)))
        x0_3 = self.conv0_3(align_and_concat(x0_0, x0_1, x0_2, self.Up(x1_2)))

        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_1 = self.conv3_1(align_and_concat(x3_0, self.Up(x4_0)))
        x2_2 = self.conv2_2(align_and_concat(x2_0, x2_1, self.Up(x3_1)))
        x1_3 = self.conv1_3(align_and_concat(x1_0, x1_1, x1_2, self.Up(x2_2)))
        x0_4 = self.conv0_4(align_and_concat(x0_0, x0_1, x0_2, x0_3, self.Up(x1_3)))
